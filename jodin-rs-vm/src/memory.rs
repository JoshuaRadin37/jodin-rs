...
impl Stack {
    ...
    pub fn pop_byte(&mut self) -> u8 {
        unsafe { self.memory.pop().unwrap().byte }
    }

    pub fn pop_short(&mut self) -> u16 {
        unsafe { self.memory.pop().unwrap().short }
    }

    pub fn pop_int(&mut self) -> u32 {
        unsafe { self.memory.pop().unwrap().int }
    }

    pub fn pop_long(&mut self) -> u64 {
        unsafe { self.memory.pop().unwrap().long }
    }

    pub fn pop_float(&mut self) -> f32 {
        unsafe { self.memory.pop().unwrap().float }
    }

    pub fn pop_double(&mut self) -> f64 {
        unsafe { self.memory.pop().unwrap().double }
    }
}
