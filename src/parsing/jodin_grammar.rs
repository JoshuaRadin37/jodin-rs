...
// sha3: a58e4f26fcc83f334eb9ef8cfc3356b13bcaaffd337554c99aa4b96a3dff3ab
...
mod __parse__Identifier {
    ...
    {
        match *__token {
            Tok::Namespaced if true => Some(0),
            Tok::Constant(_) if true => Some(1),
            Tok::Identifier(_) if true => Some(2),
            Tok::StringLiteral(_) if true => Some(3),
            _ => None,
        }
    }
    ...
    ) -> __Symbol<'input>
    {
        match __token_index {
            ...
            1 | 2 | 3 => match __token {
                Tok::Constant(__tok0) | Tok::Identifier(__tok0) | Tok::StringLiteral(__tok0) if true => __Symbol::Variant1(__tok0),
                _ => unreachable!(),
            },
            _ => unreachable!(),
        }
    ...
    {
        // Identifier = SingleIdentifier => ActionFn(5);
        let __sym0 = __pop_Variant2(__symbols);
        let __start = __sym0.0.clone();
        let __end = __sym0.2.clone();
    ...
    {
        // Identifier = Identifier, "::", SingleIdentifier => ActionFn(6);
    ...
    {
        // SingleIdentifier = t_identifier => ActionFn(4);
        let __sym0 = __pop_Variant1(__symbols);
        let __start = __sym0.0.clone();
        let __end = __sym0.2.clone();
    ...
    {
        // String = t_string_literal => ActionFn(7);
        let __sym0 = __pop_Variant1(__symbols);
        let __start = __sym0.0.clone();
        let __end = __sym0.2.clone();
...
>(
    input: &'input str,
    (_, p, _): (usize, Identifier, usize),
    (_, _, _): (usize, Tok<'input>, usize),
    (_, s, _): (usize, Identifier, usize),
) -> Identifier
{
    Identifier::new_concat(p, s)
}
